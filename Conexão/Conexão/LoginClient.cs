...
namespace Auth
{
    public class LoginClient : IDisposable
    {
        ...
        public void Start()
        {
            ...
            Seed = (ushort)new Random().Next((int)SessionId, 0x7FFF);
        ...
        private void ConnectionCheck()
        {
            ...
            {
                Close(0, true);
                Logger.warning("Conexão destruída devido a nenhuma resposta."); // Conexão Não Da Resposta Ápós Da Erro De Socket E Logo Após Volta Ao Normal
            }
        }
        ...
        private void FirstPacketCheck(ushort packetId)
        {
            if (firstPacketId == 0)
            {
                ...
                {
                    Close(0, true);
                    Logger.warning("Conexão destruída devido ao primeiro pacote desconhecido. [" + packetId + "]");
                }
            }
        }
        private void RunPacket(byte[] buff, byte[] simple)
        {
            ...
            Seed = (ushort)(Seed * 214013 + 2531011 >> 16 & short.MaxValue);
            UInt16 SessionSeed = BitConverter.ToUInt16(buff, 2);
            if (Seed != SessionSeed)
            {
                Console.ForegroundColor = ConsoleColor.Yellow;
                Console.WriteLine("[Auth] Bot Bloqueado, Conexão Finalizada.");
                Close(20000, true);
                return;
            }
