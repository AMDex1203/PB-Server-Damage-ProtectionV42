...
/*namespace PointBlank
{
    public class FirewallAuth
    {
        ...
        public static void LoadInstances(string processName, int sessionsBattle)
        {
            ...
            {
                FirewallRuleNameBattleUDP[i] = $"{processName} Allow Battle UDP Connections " + Battle.config.Config.hosPort + i;
            }
        }
        ...
        public static void CreateRuleAllow(string ruleName, string ip, int port, NET_FW_IP_PROTOCOL_ protocol)
        {
            ...
            try
            {
                ...
                Console.WriteLine($" [Firewall] Allow {(protocol == NET_FW_IP_PROTOCOL_.NET_FW_IP_PROTOCOL_TCP ? "Tcp" : protocol == NET_FW_IP_PROTOCOL_.NET_FW_IP_PROTOCOL_UDP ? "Udp" : "Unknown")} connection rule on port {port} has been registered in windows firewall.");
                firewallPolicy = null;
                inboundRule = null;
            }
            catch (Exception ex)
            {
                Core.Logger.Exception(ex);
            }
        }
        ...
        public static void AddRuleUdp(string address, int sessionPort) //RemoteIP
        {
            try
            {
                ...
                foreach (INetFwRule rule in firewallPolicy.Rules)
                {
                    if (rule.Name.IndexOf(FirewallRuleNameBattleUDP[sessionPort - Battle.config.Config.hosPort]) != -1 && !rule.RemoteAddresses.Contains(address))
                    {
        ...
        public static void RemoveRuleUdp(string address, int sessionPort) //RemoteIP
        {
            try
            {
                ...
                foreach (INetFwRule rule in firewallPolicy.Rules)
                {
                    if (rule.Name.IndexOf(FirewallRuleNameBattleUDP[sessionPort - Auth.configGA.authPort]) != -1 && rule.RemoteAddresses.Contains(address))
                    {
